...
namespace ComputerManagment
{
    ...
    public sealed class POTSModemToSerialPort
    {
        ...
        public static POTSModemToSerialPort[] Retrieve(string remote, string username, string password)
        {
        ...
        public static POTSModemToSerialPort[] Retrieve()
        {
            var managementScope = new ManagementScope(new ManagementPath("root\\cimv2"));
            return Retrieve(managementScope);
        }

        public static POTSModemToSerialPort[] Retrieve(ManagementScope managementScope)
        {
            var objectQuery = new ObjectQuery("SELECT * FROM Win32_POTSModemToSerialPort");
            var objectSearcher = new ManagementObjectSearcher(managementScope, objectQuery);
            var objectCollection = objectSearcher.Get();

            var list = new List<POTSModemToSerialPort>();

            foreach (ManagementObject managementObject in objectCollection)
                list.Add(new POTSModemToSerialPort
                {
