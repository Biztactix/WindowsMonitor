...
namespace ComputerManagment
{
    ...
    public sealed class PCMCIAController
    {
        ...
        public static PCMCIAController[] Retrieve(string remote, string username, string password)
        {
        ...
        public static PCMCIAController[] Retrieve()
        {
            var managementScope = new ManagementScope(new ManagementPath("root\\cimv2"));
            return Retrieve(managementScope);
        }

        public static PCMCIAController[] Retrieve(ManagementScope managementScope)
        {
            var objectQuery = new ObjectQuery("SELECT * FROM Win32_PCMCIAController");
            var objectSearcher = new ManagementObjectSearcher(managementScope, objectQuery);
            var objectCollection = objectSearcher.Get();

            var list = new List<PCMCIAController>();

            foreach (ManagementObject managementObject in objectCollection)
                list.Add(new PCMCIAController
                {
