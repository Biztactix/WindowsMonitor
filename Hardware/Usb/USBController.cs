...
namespace ComputerManagment
{
    ...
    public sealed class USBController
    {
        ...
        public static USBController[] Retrieve(string remote, string username, string password)
        {
        ...
        public static USBController[] Retrieve()
        {
            var managementScope = new ManagementScope(new ManagementPath("root\\cimv2"));
            return Retrieve(managementScope);
        }

        public static USBController[] Retrieve(ManagementScope managementScope)
        {
            var objectQuery = new ObjectQuery("SELECT * FROM Win32_USBController");
            var objectSearcher = new ManagementObjectSearcher(managementScope, objectQuery);
            var objectCollection = objectSearcher.Get();

            var list = new List<USBController>();

            foreach (ManagementObject managementObject in objectCollection)
                list.Add(new USBController
                {
