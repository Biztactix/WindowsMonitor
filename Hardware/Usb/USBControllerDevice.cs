...
namespace ComputerManagment
{
    ...
    public sealed class USBControllerDevice
    {
        ...
        public static USBControllerDevice[] Retrieve(string remote, string username, string password)
        {
        ...
        public static USBControllerDevice[] Retrieve()
        {
            var managementScope = new ManagementScope(new ManagementPath("root\\cimv2"));
            return Retrieve(managementScope);
        }

        public static USBControllerDevice[] Retrieve(ManagementScope managementScope)
        {
            var objectQuery = new ObjectQuery("SELECT * FROM Win32_USBControllerDevice");
            var objectSearcher = new ManagementObjectSearcher(managementScope, objectQuery);
            var objectCollection = objectSearcher.Get();

            var list = new List<USBControllerDevice>();

            foreach (ManagementObject managementObject in objectCollection)
                list.Add(new USBControllerDevice
                {
