...
namespace ComputerManagment
{
    ...
    public sealed class MemoryDevice
    {
        ...
        public static MemoryDevice[] Retrieve(string remote, string username, string password)
        {
        ...
        public static MemoryDevice[] Retrieve()
        {
            var managementScope = new ManagementScope(new ManagementPath("root\\cimv2"));
            return Retrieve(managementScope);
        }

        public static MemoryDevice[] Retrieve(ManagementScope managementScope)
        {
            var objectQuery = new ObjectQuery("SELECT * FROM Win32_MemoryDevice");
            var objectSearcher = new ManagementObjectSearcher(managementScope, objectQuery);
            var objectCollection = objectSearcher.Get();

            var list = new List<MemoryDevice>();

            foreach (ManagementObject managementObject in objectCollection)
                list.Add(new MemoryDevice
                {
