...
namespace ComputerManagment
{
    ...
    public sealed class PhysicalMemoryArray
    {
        ...
        public static PhysicalMemoryArray[] Retrieve(string remote, string username, string password)
        {
        ...
        public static PhysicalMemoryArray[] Retrieve()
        {
            var managementScope = new ManagementScope(new ManagementPath("root\\cimv2"));
            return Retrieve(managementScope);
        }

        public static PhysicalMemoryArray[] Retrieve(ManagementScope managementScope)
        {
            var objectQuery = new ObjectQuery("SELECT * FROM Win32_PhysicalMemoryArray");
            var objectSearcher = new ManagementObjectSearcher(managementScope, objectQuery);
            var objectCollection = objectSearcher.Get();

            var list = new List<PhysicalMemoryArray>();

            foreach (ManagementObject managementObject in objectCollection)
                list.Add(new PhysicalMemoryArray
                {
                    ...
                    MaxCapacity = (uint) managementObject.Properties["MaxCapacity"].Value,
                    MaxCapacityEx = (ulong) managementObject.Properties["MaxCapacityEx"].Value,
                    MemoryDevices = (ushort) managementObject.Properties["MemoryDevices"].Value,
                    MemoryErrorCorrection = (ushort) managementObject.Properties["MemoryErrorCorrection"].Value,
                    ...
                    Use = (ushort) managementObject.Properties["Use"].Value,
