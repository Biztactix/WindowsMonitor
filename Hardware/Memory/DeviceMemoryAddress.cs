...
namespace ComputerManagment
{
    ...
    public sealed class DeviceMemoryAddress
    {
        ...
        public static DeviceMemoryAddress[] Retrieve(string remote, string username, string password)
        {
        ...
        public static DeviceMemoryAddress[] Retrieve()
        {
            var managementScope = new ManagementScope(new ManagementPath("root\\cimv2"));
            return Retrieve(managementScope);
        }

        public static DeviceMemoryAddress[] Retrieve(ManagementScope managementScope)
        {
            var objectQuery = new ObjectQuery("SELECT * FROM Win32_DeviceMemoryAddress");
            var objectSearcher = new ManagementObjectSearcher(managementScope, objectQuery);
            var objectCollection = objectSearcher.Get();

            var list = new List<DeviceMemoryAddress>();

            foreach (ManagementObject managementObject in objectCollection)
                list.Add(new DeviceMemoryAddress
                {
