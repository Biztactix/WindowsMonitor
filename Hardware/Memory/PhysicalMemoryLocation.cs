...
namespace ComputerManagment
{
    ...
    public sealed class PhysicalMemoryLocation
    {
        ...
        public static PhysicalMemoryLocation[] Retrieve(string remote, string username, string password)
        {
        ...
        public static PhysicalMemoryLocation[] Retrieve()
        {
            var managementScope = new ManagementScope(new ManagementPath("root\\cimv2"));
            return Retrieve(managementScope);
        }

        public static PhysicalMemoryLocation[] Retrieve(ManagementScope managementScope)
        {
            var objectQuery = new ObjectQuery("SELECT * FROM Win32_PhysicalMemoryLocation");
            var objectSearcher = new ManagementObjectSearcher(managementScope, objectQuery);
            var objectCollection = objectSearcher.Get();

            var list = new List<PhysicalMemoryLocation>();

            foreach (ManagementObject managementObject in objectCollection)
                list.Add(new PhysicalMemoryLocation
                {
