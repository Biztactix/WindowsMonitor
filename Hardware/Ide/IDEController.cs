...
namespace ComputerManagment
{
    ...
    public sealed class IDEController
    {
        ...
        public static IDEController[] Retrieve(string remote, string username, string password)
        {
        ...
        public static IDEController[] Retrieve()
        {
            var managementScope = new ManagementScope(new ManagementPath("root\\cimv2"));
            return Retrieve(managementScope);
        }

        public static IDEController[] Retrieve(ManagementScope managementScope)
        {
            var objectQuery = new ObjectQuery("SELECT * FROM Win32_IDEController");
            var objectSearcher = new ManagementObjectSearcher(managementScope, objectQuery);
            var objectCollection = objectSearcher.Get();

            var list = new List<IDEController>();

            foreach (ManagementObject managementObject in objectCollection)
                list.Add(new IDEController
                {
