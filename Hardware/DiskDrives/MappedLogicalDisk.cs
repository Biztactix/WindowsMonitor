...
namespace ComputerManagment
{
    ...
    public sealed class MappedLogicalDisk
    {
        ...
        public static MappedLogicalDisk[] Retrieve(string remote, string username, string password)
        {
        ...
        public static MappedLogicalDisk[] Retrieve()
        {
            var managementScope = new ManagementScope(new ManagementPath("root\\cimv2"));
            return Retrieve(managementScope);
        }

        public static MappedLogicalDisk[] Retrieve(ManagementScope managementScope)
        {
            var objectQuery = new ObjectQuery("SELECT * FROM Win32_MappedLogicalDisk");
            var objectSearcher = new ManagementObjectSearcher(managementScope, objectQuery);
            var objectCollection = objectSearcher.Get();

            var list = new List<MappedLogicalDisk>();

            foreach (ManagementObject managementObject in objectCollection)
                list.Add(new MappedLogicalDisk
                {
                    ...
                    SessionID = (string) managementObject.Properties["SessionID"].Value,
