...
namespace ComputerManagment
{
    ...
    public sealed class SMBIOSMemory
    {
        ...
        public static SMBIOSMemory[] Retrieve(string remote, string username, string password)
        {
        ...
        public static SMBIOSMemory[] Retrieve()
        {
            var managementScope = new ManagementScope(new ManagementPath("root\\cimv2"));
            return Retrieve(managementScope);
        }

        public static SMBIOSMemory[] Retrieve(ManagementScope managementScope)
        {
            var objectQuery = new ObjectQuery("SELECT * FROM Win32_SMBIOSMemory");
            var objectSearcher = new ManagementObjectSearcher(managementScope, objectQuery);
            var objectCollection = objectSearcher.Get();

            var list = new List<SMBIOSMemory>();

            foreach (ManagementObject managementObject in objectCollection)
                list.Add(new SMBIOSMemory
                {
