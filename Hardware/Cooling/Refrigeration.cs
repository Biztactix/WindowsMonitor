...
namespace ComputerManagment
{
    /// <summary>
    /// </summary>
    public sealed class Refrigeration
    {
        ...
        public static Refrigeration[] Retrieve(string remote, string username, string password)
        {
        ...
        public static Refrigeration[] Retrieve()
        {
            var managementScope = new ManagementScope(new ManagementPath("root\\cimv2"));
            return Retrieve(managementScope);
        }

        public static Refrigeration[] Retrieve(ManagementScope managementScope)
        {
            var objectQuery = new ObjectQuery("SELECT * FROM Win32_Refrigeration");
            var objectSearcher = new ManagementObjectSearcher(managementScope, objectQuery);
            var objectCollection = objectSearcher.Get();

            var list = new List<Refrigeration>();

            foreach (ManagementObject managementObject in objectCollection)
                list.Add(new Refrigeration
                {
