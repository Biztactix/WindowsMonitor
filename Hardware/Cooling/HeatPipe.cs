...
namespace ComputerManagment
{
    ...
    public sealed class HeatPipe
    {
        ...
        public static HeatPipe[] Retrieve(string remote, string username, string password)
        {
        ...
        public static HeatPipe[] Retrieve()
        {
            var managementScope = new ManagementScope(new ManagementPath("root\\cimv2"));
            return Retrieve(managementScope);
        }

        public static HeatPipe[] Retrieve(ManagementScope managementScope)
        {
            var objectQuery = new ObjectQuery("SELECT * FROM Win32_HeatPipe");
            var objectSearcher = new ManagementObjectSearcher(managementScope, objectQuery);
            var objectCollection = objectSearcher.Get();

            var list = new List<HeatPipe>();

            foreach (ManagementObject managementObject in objectCollection)
                list.Add(new HeatPipe
                {
