...
namespace ComputerManagment
{
    ...
    public sealed class CurrentProbe
    {
        ...
        public static CurrentProbe[] Retrieve(string remote, string username, string password)
        {
        ...
        public static CurrentProbe[] Retrieve()
        {
            var managementScope = new ManagementScope(new ManagementPath("root\\cimv2"));
            return Retrieve(managementScope);
        }

        public static CurrentProbe[] Retrieve(ManagementScope managementScope)
        {
            var objectQuery = new ObjectQuery("SELECT * FROM Win32_CurrentProbe");
            var objectSearcher = new ManagementObjectSearcher(managementScope, objectQuery);
            var objectCollection = objectSearcher.Get();

            var list = new List<CurrentProbe>();

            foreach (ManagementObject managementObject in objectCollection)
                list.Add(new CurrentProbe
                {
