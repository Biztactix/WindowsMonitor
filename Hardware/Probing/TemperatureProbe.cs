...
namespace ComputerManagment
{
    ...
    public sealed class TemperatureProbe
    {
        ...
        public static TemperatureProbe[] Retrieve(string remote, string username, string password)
        {
        ...
        public static TemperatureProbe[] Retrieve()
        {
            var managementScope = new ManagementScope(new ManagementPath("root\\cimv2"));
            return Retrieve(managementScope);
        }

        public static TemperatureProbe[] Retrieve(ManagementScope managementScope)
        {
            var objectQuery = new ObjectQuery("SELECT * FROM Win32_TemperatureProbe");
            var objectSearcher = new ManagementObjectSearcher(managementScope, objectQuery);
            var objectCollection = objectSearcher.Get();

            var list = new List<TemperatureProbe>();

            foreach (ManagementObject managementObject in objectCollection)
                list.Add(new TemperatureProbe
                {
