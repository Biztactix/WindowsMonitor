...
namespace ComputerManagment
{
    ...
    public sealed class SecuritySettingAuditing
    {
        ...
        public static SecuritySettingAuditing[] Retrieve(string remote, string username, string password)
        {
        ...
        public static SecuritySettingAuditing[] Retrieve()
        {
            var managementScope = new ManagementScope(new ManagementPath("root\\cimv2"));
            return Retrieve(managementScope);
        }

        public static SecuritySettingAuditing[] Retrieve(ManagementScope managementScope)
        {
            var objectQuery = new ObjectQuery("SELECT * FROM Win32_SecuritySettingAuditing");
            var objectSearcher = new ManagementObjectSearcher(managementScope, objectQuery);
            var objectCollection = objectSearcher.Get();

            var list = new List<SecuritySettingAuditing>();

            foreach (ManagementObject managementObject in objectCollection)
                list.Add(new SecuritySettingAuditing
                {
