...
namespace ComputerManagment
{
    ...
    public sealed class SecuritySettingAccess
    {
        ...
        public static SecuritySettingAccess[] Retrieve(string remote, string username, string password)
        {
        ...
        public static SecuritySettingAccess[] Retrieve()
        {
            var managementScope = new ManagementScope(new ManagementPath("root\\cimv2"));
            return Retrieve(managementScope);
        }

        public static SecuritySettingAccess[] Retrieve(ManagementScope managementScope)
        {
            var objectQuery = new ObjectQuery("SELECT * FROM Win32_SecuritySettingAccess");
            var objectSearcher = new ManagementObjectSearcher(managementScope, objectQuery);
            var objectCollection = objectSearcher.Get();

            var list = new List<SecuritySettingAccess>();

            foreach (ManagementObject managementObject in objectCollection)
                list.Add(new SecuritySettingAccess
                {
