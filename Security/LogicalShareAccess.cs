...
namespace ComputerManagment
{
    ...
    public sealed class LogicalShareAccess
    {
        ...
        public static LogicalShareAccess[] Retrieve(string remote, string username, string password)
        {
        ...
        public static LogicalShareAccess[] Retrieve()
        {
            var managementScope = new ManagementScope(new ManagementPath("root\\cimv2"));
            return Retrieve(managementScope);
        }

        public static LogicalShareAccess[] Retrieve(ManagementScope managementScope)
        {
            var objectQuery = new ObjectQuery("SELECT * FROM Win32_LogicalShareAccess");
            var objectSearcher = new ManagementObjectSearcher(managementScope, objectQuery);
            var objectCollection = objectSearcher.Get();

            var list = new List<LogicalShareAccess>();

            foreach (ManagementObject managementObject in objectCollection)
                list.Add(new LogicalShareAccess
                {
