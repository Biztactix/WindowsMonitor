...
public UInt64 TIME_CREATED { get; private set; }
		...
		public static ProcessStopTrace[] Retrieve(string remote, string username, string password)
		{
		...
		public static ProcessStopTrace[] Retrieve()
		{
			var managementScope = new ManagementScope(new ManagementPath("root\\cimv2"));
			return Retrieve(managementScope);
		}

		public static ProcessStopTrace[] Retrieve(ManagementScope managementScope)
		{
		    var objectQuery = new ObjectQuery("SELECT * FROM Win32_ProcessStopTrace");
            var objectSearcher = new ManagementObjectSearcher(managementScope, objectQuery);
            var objectCollection = objectSearcher.Get();

			var list = new List<ProcessStopTrace>();

            foreach (ManagementObject managementObject in objectCollection)
            {
				list.Add(new ProcessStopTrace
				{
					ExitStatus = (UInt32) managementObject.Properties["ExitStatus"].Value,
