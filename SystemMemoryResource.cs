...
namespace ComputerManagment
{
    ...
    public sealed class SystemMemoryResource
    {
        ...
        public static SystemMemoryResource[] Retrieve(string remote, string username, string password)
        {
        ...
        public static SystemMemoryResource[] Retrieve()
        {
            var managementScope = new ManagementScope(new ManagementPath("root\\cimv2"));
            return Retrieve(managementScope);
        }

        public static SystemMemoryResource[] Retrieve(ManagementScope managementScope)
        {
            var objectQuery = new ObjectQuery("SELECT * FROM Win32_SystemMemoryResource");
            var objectSearcher = new ManagementObjectSearcher(managementScope, objectQuery);
            var objectCollection = objectSearcher.Get();

            var list = new List<SystemMemoryResource>();

            foreach (ManagementObject managementObject in objectCollection)
                list.Add(new SystemMemoryResource
                {
