...
namespace WindowsMonitor
{
    ...
    public sealed class SQL_2016
    {
        ...
        public static IEnumerable<SQL_2016> Retrieve(string remote, string username, string password)
        {
        ...
        public static IEnumerable<SQL_2016> Retrieve()
        {
            var managementScope = new ManagementScope(new ManagementPath("root\\cimv2"));
            return Retrieve(managementScope);
        }

        public static IEnumerable<SQL_2016> Retrieve(ManagementScope managementScope)
        {
            var objectQuery = new ObjectQuery("SELECT * FROM SQL_2016");
            var objectSearcher = new ManagementObjectSearcher(managementScope, objectQuery);
            var objectCollection = objectSearcher.Get();

            foreach (ManagementObject managementObject in objectCollection)
                yield return new SQL_2016
                {
