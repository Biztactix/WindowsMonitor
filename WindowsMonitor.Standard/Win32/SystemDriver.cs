...
namespace WindowsMonitor.Win32
{
    /// <summary>
    /// </summary>
    public sealed class SystemDriver
    {
        ...
        public static IEnumerable<SystemDriver> Retrieve(string remote, string username, string password)
        {
        ...
        public static IEnumerable<SystemDriver> Retrieve()
        {
            var managementScope = new ManagementScope(new ManagementPath("root\\cimv2"));
            return Retrieve(managementScope);
        }

        public static IEnumerable<SystemDriver> Retrieve(ManagementScope managementScope)
        {
            var objectQuery = new ObjectQuery("SELECT * FROM Win32_SystemDriver");
            var objectSearcher = new ManagementObjectSearcher(managementScope, objectQuery);
            var objectCollection = objectSearcher.Get();

            foreach (ManagementObject managementObject in objectCollection)
                yield return new SystemDriver
                {
		 ...
		 ErrorControl = (string) (managementObject.Properties["ErrorControl"]?.Value ?? default(string)),
		 ExitCode = (uint) (managementObject.Properties["ExitCode"]?.Value ?? default(uint)),
		 InstallDate = (DateTime) (managementObject.Properties["InstallDate"]?.Value ?? default(DateTime)),
		 Name = (string) (managementObject.Properties["Name"]?.Value ?? default(string)),
		 PathName = (string) (managementObject.Properties["PathName"]?.Value ?? default(string)),
		 ServiceSpecificExitCode = (uint) (managementObject.Properties["ServiceSpecificExitCode"]?.Value ?? default(uint)),
		 ServiceType = (string) (managementObject.Properties["ServiceType"]?.Value ?? default(string)),
		 ...
		 State = (string) (managementObject.Properties["State"]?.Value ?? default(string)),
