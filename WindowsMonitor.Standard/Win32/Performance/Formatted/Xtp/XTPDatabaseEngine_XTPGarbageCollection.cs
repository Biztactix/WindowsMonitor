...
namespace WindowsMonitor.Performance.Formatted
{
    /// <summary>
    /// </summary>
    public sealed class XTPDatabaseEngine_XTPGarbageCollection
    {
        ...
        public static IEnumerable<XTPDatabaseEngine_XTPGarbageCollection> Retrieve(string remote, string username, string password)
        {
        ...
        public static IEnumerable<XTPDatabaseEngine_XTPGarbageCollection> Retrieve()
        {
            var managementScope = new ManagementScope(new ManagementPath("root\\cimv2"));
            return Retrieve(managementScope);
        }

        public static IEnumerable<XTPDatabaseEngine_XTPGarbageCollection> Retrieve(ManagementScope managementScope)
        {
            var objectQuery = new ObjectQuery("SELECT * FROM Win32_PerfFormattedData_XTPDatabaseEngine_XTPGarbageCollection");
            var objectSearcher = new ManagementObjectSearcher(managementScope, objectQuery);
            var objectCollection = objectSearcher.Get();

            foreach (ManagementObject managementObject in objectCollection)
                yield return new XTPDatabaseEngine_XTPGarbageCollection
                {
