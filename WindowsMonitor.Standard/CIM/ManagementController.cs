...
namespace WindowsMonitor.CIM
{
    ...
    public sealed class ManagementController
    {
        ...
        public static IEnumerable<ManagementController> Retrieve(string remote, string username, string password)
        {
        ...
        public static IEnumerable<ManagementController> Retrieve()
        {
            var managementScope = new ManagementScope(new ManagementPath("root\\cimv2"));
            return Retrieve(managementScope);
        }

        public static IEnumerable<ManagementController> Retrieve(ManagementScope managementScope)
        {
            var objectQuery = new ObjectQuery("SELECT * FROM CIM_ManagementController");
            var objectSearcher = new ManagementObjectSearcher(managementScope, objectQuery);
            var objectCollection = objectSearcher.Get();

            foreach (ManagementObject managementObject in objectCollection)
                yield return new ManagementController
                {
