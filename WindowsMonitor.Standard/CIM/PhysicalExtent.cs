...
namespace WindowsMonitor.CIM
{
    /// <summary>
    /// </summary>
    public sealed class PhysicalExtent
    {
		...
		public ulong UnitsBeforeCheckDataInterleave { get; private set; }
		public ulong UnitsOfCheckData { get; private set; }
		public ulong UnitsOfUserData { get; private set; }

        public static IEnumerable<PhysicalExtent> Retrieve(string remote, string username, string password)
        {
        ...
        public static IEnumerable<PhysicalExtent> Retrieve()
        {
            var managementScope = new ManagementScope(new ManagementPath("root\\cimv2"));
            return Retrieve(managementScope);
        }

        public static IEnumerable<PhysicalExtent> Retrieve(ManagementScope managementScope)
        {
            var objectQuery = new ObjectQuery("SELECT * FROM CIM_PhysicalExtent");
            var objectSearcher = new ManagementObjectSearcher(managementScope, objectQuery);
            var objectCollection = objectSearcher.Get();

            foreach (ManagementObject managementObject in objectCollection)
                yield return new PhysicalExtent
                {
		 ...
		 UnitsBeforeCheckDataInterleave = (ulong) (managementObject.Properties["UnitsBeforeCheckDataInterleave"]?.Value ?? default(ulong)),
		 UnitsOfCheckData = (ulong) (managementObject.Properties["UnitsOfCheckData"]?.Value ?? default(ulong)),
		 UnitsOfUserData = (ulong) (managementObject.Properties["UnitsOfUserData"]?.Value ?? default(ulong))
                };
        }
    }
}