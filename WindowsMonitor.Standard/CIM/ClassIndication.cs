...
namespace WindowsMonitor.CIM
{
    /// <summary>
    /// </summary>
    public sealed class ClassIndication
    {
        ...
        public static IEnumerable<ClassIndication> Retrieve(string remote, string username, string password)
        {
        ...
        public static IEnumerable<ClassIndication> Retrieve()
        {
            var managementScope = new ManagementScope(new ManagementPath("root\\cimv2"));
            return Retrieve(managementScope);
        }

        public static IEnumerable<ClassIndication> Retrieve(ManagementScope managementScope)
        {
            var objectQuery = new ObjectQuery("SELECT * FROM CIM_ClassIndication");
            var objectSearcher = new ManagementObjectSearcher(managementScope, objectQuery);
            var objectCollection = objectSearcher.Get();

            foreach (ManagementObject managementObject in objectCollection)
                yield return new ClassIndication
                {
