...
namespace WindowsMonitor.Msft
{
    /// <summary>
    /// </summary>
    public sealed class WmiProvider_CancelQuery_Pre
    {
        ...
        public static IEnumerable<WmiProvider_CancelQuery_Pre> Retrieve(string remote, string username, string password)
        {
        ...
        public static IEnumerable<WmiProvider_CancelQuery_Pre> Retrieve()
        {
            var managementScope = new ManagementScope(new ManagementPath("root\\cimv2"));
            return Retrieve(managementScope);
        }

        public static IEnumerable<WmiProvider_CancelQuery_Pre> Retrieve(ManagementScope managementScope)
        {
            var objectQuery = new ObjectQuery("SELECT * FROM Msft_WmiProvider_CancelQuery_Pre");
            var objectSearcher = new ManagementObjectSearcher(managementScope, objectQuery);
            var objectCollection = objectSearcher.Get();

            foreach (ManagementObject managementObject in objectCollection)
                yield return new WmiProvider_CancelQuery_Pre
                {
