...
namespace WindowsMonitor.CIM
{
    ...
    public sealed class DiskPartition
    {
        ...
        public static IEnumerable<DiskPartition> Retrieve(string remote, string username, string password)
        {
        ...
        public static IEnumerable<DiskPartition> Retrieve()
        {
            var managementScope = new ManagementScope(new ManagementPath("root\\cimv2"));
            return Retrieve(managementScope);
        }

        public static IEnumerable<DiskPartition> Retrieve(ManagementScope managementScope)
        {
            var objectQuery = new ObjectQuery("SELECT * FROM CIM_DiskPartition");
            var objectSearcher = new ManagementObjectSearcher(managementScope, objectQuery);
            var objectCollection = objectSearcher.Get();

            foreach (ManagementObject managementObject in objectCollection)
                yield return new DiskPartition
                {
