...
namespace WindowsMonitor.CIM
{
    ...
    public sealed class Keyboard
    {
        ...
        public static IEnumerable<Keyboard> Retrieve(string remote, string username, string password)
        {
        ...
        public static IEnumerable<Keyboard> Retrieve()
        {
            var managementScope = new ManagementScope(new ManagementPath("root\\cimv2"));
            return Retrieve(managementScope);
        }

        public static IEnumerable<Keyboard> Retrieve(ManagementScope managementScope)
        {
            var objectQuery = new ObjectQuery("SELECT * FROM CIM_Keyboard");
            var objectSearcher = new ManagementObjectSearcher(managementScope, objectQuery);
            var objectCollection = objectSearcher.Get();

            foreach (ManagementObject managementObject in objectCollection)
                yield return new Keyboard
                {
		 ...
		 Layout = (string) (managementObject.Properties["Layout"]?.Value ?? default(string)),
