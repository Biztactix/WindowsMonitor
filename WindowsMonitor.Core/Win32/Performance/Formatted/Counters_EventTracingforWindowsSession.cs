...
namespace WindowsMonitor.Win32.Performance.Formatted
{
    /// <summary>
    /// </summary>
    public sealed class CountersEventTracingforWindowsSession
    {
        ...
        public static IEnumerable<CountersEventTracingforWindowsSession> Retrieve(string remote, string username, string password)
        {
        ...
        public static IEnumerable<CountersEventTracingforWindowsSession> Retrieve()
        {
            var managementScope = new ManagementScope(new ManagementPath("root\\cimv2"));
            return Retrieve(managementScope);
        }

        public static IEnumerable<CountersEventTracingforWindowsSession> Retrieve(ManagementScope managementScope)
        {
            var objectQuery = new ObjectQuery("SELECT * FROM Win32_PerfFormattedData_Counters_EventTracingforWindowsSession");
            var objectSearcher = new ManagementObjectSearcher(managementScope, objectQuery);
            var objectCollection = objectSearcher.Get();

            foreach (ManagementObject managementObject in objectCollection)
                yield return new CountersEventTracingforWindowsSession
                {
