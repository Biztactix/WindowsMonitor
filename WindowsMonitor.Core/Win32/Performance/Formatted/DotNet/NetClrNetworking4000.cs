...
namespace WindowsMonitor.Win32.Performance.Formatted.DotNet
{
    /// <summary>
    /// </summary>
    public sealed class NetClrNetworking4000
    {
        ...
        public static IEnumerable<NetClrNetworking4000> Retrieve(string remote, string username, string password)
        {
        ...
        public static IEnumerable<NetClrNetworking4000> Retrieve()
        {
            var managementScope = new ManagementScope(new ManagementPath("root\\cimv2"));
            return Retrieve(managementScope);
        }

        public static IEnumerable<NetClrNetworking4000> Retrieve(ManagementScope managementScope)
        {
            var objectQuery = new ObjectQuery("SELECT * FROM Win32_PerfFormattedData_NETCLRNetworking4000_NETCLRNetworking4000");
            var objectSearcher = new ManagementObjectSearcher(managementScope, objectQuery);
            var objectCollection = objectSearcher.Get();

            foreach (ManagementObject managementObject in objectCollection)
                yield return new NetClrNetworking4000
                {
