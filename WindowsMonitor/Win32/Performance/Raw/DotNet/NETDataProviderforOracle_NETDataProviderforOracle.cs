...
namespace WindowsMonitor.Win32.Performance.Raw.DotNet
{
    /// <summary>
    /// </summary>
    public sealed class NetDataProviderforOracleNetDataProviderforOracle
    {
        ...
        public static IEnumerable<NetDataProviderforOracleNetDataProviderforOracle> Retrieve(string remote, string username, string password)
        {
        ...
        public static IEnumerable<NetDataProviderforOracleNetDataProviderforOracle> Retrieve()
        {
            var managementScope = new ManagementScope(new ManagementPath("root\\cimv2"));
            return Retrieve(managementScope);
        }

        public static IEnumerable<NetDataProviderforOracleNetDataProviderforOracle> Retrieve(ManagementScope managementScope)
        {
            var objectQuery = new ObjectQuery("SELECT * FROM Win32_PerfRawData_NETDataProviderforOracle_NETDataProviderforOracle");
            var objectSearcher = new ManagementObjectSearcher(managementScope, objectQuery);
            var objectCollection = objectSearcher.Get();

            foreach (ManagementObject managementObject in objectCollection)
                yield return new NetDataProviderforOracleNetDataProviderforOracle
                {
