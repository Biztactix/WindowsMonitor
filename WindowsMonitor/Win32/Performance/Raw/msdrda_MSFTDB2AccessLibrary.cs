...
namespace WindowsMonitor.Performance.Raw
{
    /// <summary>
    /// </summary>
    public sealed class msdrda_MSFTDB2AccessLibrary
    {
        ...
        public static IEnumerable<msdrda_MSFTDB2AccessLibrary> Retrieve(string remote, string username, string password)
        {
        ...
        public static IEnumerable<msdrda_MSFTDB2AccessLibrary> Retrieve()
        {
            var managementScope = new ManagementScope(new ManagementPath("root\\cimv2"));
            return Retrieve(managementScope);
        }

        public static IEnumerable<msdrda_MSFTDB2AccessLibrary> Retrieve(ManagementScope managementScope)
        {
            ...
            foreach (ManagementObject managementObject in objectCollection)
                yield return new msdrda_MSFTDB2AccessLibrary
                {
                     AvgHostProcessingTime = (uint) (managementObject.Properties["AvgHostProcessingTime"]?.Value ?? default(uint)),
		 AvgHostProcessingTime_Base = (uint) (managementObject.Properties["AvgHostProcessingTime_Base"]?.Value ?? default(uint)),
		 ...
		 OLEDBAvgDataAccessTime_Base = (uint) (managementObject.Properties["OLEDBAvgDataAccessTime_Base"]?.Value ?? default(uint)),
		 OLEDBAvgExecutionTime = (uint) (managementObject.Properties["OLEDBAvgExecutionTime"]?.Value ?? default(uint)),
		 OLEDBAvgExecutionTime_Base = (uint) (managementObject.Properties["OLEDBAvgExecutionTime_Base"]?.Value ?? default(uint)),
		 OLEDBAvgFetchTime = (uint) (managementObject.Properties["OLEDBAvgFetchTime"]?.Value ?? default(uint)),
		 OLEDBAvgFetchTime_Base = (uint) (managementObject.Properties["OLEDBAvgFetchTime_Base"]?.Value ?? default(uint)),
		 OLEDBAvgPrepareTime = (uint) (managementObject.Properties["OLEDBAvgPrepareTime"]?.Value ?? default(uint)),
		 OLEDBAvgPrepareTime_Base = (uint) (managementObject.Properties["OLEDBAvgPrepareTime_Base"]?.Value ?? default(uint)),
