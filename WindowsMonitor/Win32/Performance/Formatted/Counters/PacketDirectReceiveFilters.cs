...
namespace WindowsMonitor.Win32.Performance.Formatted.Counters
{
    /// <summary>
    /// </summary>
    public sealed class PacketDirectReceiveFilters
    {
        ...
        public static IEnumerable<PacketDirectReceiveFilters> Retrieve(string remote, string username, string password)
        {
        ...
        public static IEnumerable<PacketDirectReceiveFilters> Retrieve()
        {
            var managementScope = new ManagementScope(new ManagementPath("root\\cimv2"));
            return Retrieve(managementScope);
        }

        public static IEnumerable<PacketDirectReceiveFilters> Retrieve(ManagementScope managementScope)
        {
            var objectQuery = new ObjectQuery("SELECT * FROM Win32_PerfFormattedData_Counters_PacketDirectReceiveFilters");
            var objectSearcher = new ManagementObjectSearcher(managementScope, objectQuery);
            var objectCollection = objectSearcher.Get();

            foreach (ManagementObject managementObject in objectCollection)
                yield return new PacketDirectReceiveFilters
                {
