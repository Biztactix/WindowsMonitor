...
namespace WindowsMonitor.CIM
{
    /// <summary>
    /// </summary>
    public sealed class VolatileStorage
    {
        ...
        public static IEnumerable<VolatileStorage> Retrieve(string remote, string username, string password)
        {
        ...
        public static IEnumerable<VolatileStorage> Retrieve()
        {
            var managementScope = new ManagementScope(new ManagementPath("root\\cimv2"));
            return Retrieve(managementScope);
        }

        public static IEnumerable<VolatileStorage> Retrieve(ManagementScope managementScope)
        {
            var objectQuery = new ObjectQuery("SELECT * FROM CIM_VolatileStorage");
            var objectSearcher = new ManagementObjectSearcher(managementScope, objectQuery);
            var objectCollection = objectSearcher.Get();

            foreach (ManagementObject managementObject in objectCollection)
                yield return new VolatileStorage
                {
		 ...
		 Cacheable = (bool) (managementObject.Properties["Cacheable"]?.Value ?? default(bool)),
