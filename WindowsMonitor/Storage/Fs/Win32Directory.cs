...
namespace WindowsMonitor.Storage.Fs
{
    /// <summary>
    /// </summary>
    public sealed class Win32Directory
    {
        ...
        public static IEnumerable<Win32Directory> Retrieve(string remote, string username, string password)
        {
        ...
        public static IEnumerable<Win32Directory> Retrieve()
        {
            var managementScope = new ManagementScope(new ManagementPath("root\\cimv2"));
            return Retrieve(managementScope);
        }

        public static IEnumerable<Win32Directory> Retrieve(ManagementScope managementScope)
        {
            var objectQuery = new ObjectQuery("SELECT * FROM Win32_Directory");
            var objectSearcher = new ManagementObjectSearcher(managementScope, objectQuery);
            var objectCollection = objectSearcher.Get();

            foreach (ManagementObject managementObject in objectCollection)
                yield return new Win32Directory
                {
