...
namespace WindowsMonitor.WMI
{
    /// <summary>
    /// </summary>
    public sealed class Header_Extension_TypeGroup
    {
		...
		public uint KernelEventVersion { get; private set; }

        public static IEnumerable<Header_Extension_TypeGroup> Retrieve(string remote, string username, string password)
        {
        ...
        public static IEnumerable<Header_Extension_TypeGroup> Retrieve()
        {
            var managementScope = new ManagementScope(new ManagementPath("root\\wmi"));
            return Retrieve(managementScope);
        }

        public static IEnumerable<Header_Extension_TypeGroup> Retrieve(ManagementScope managementScope)
        {
            var objectQuery = new ObjectQuery("SELECT * FROM Header_Extension_TypeGroup");
            var objectSearcher = new ManagementObjectSearcher(managementScope, objectQuery);
            var objectCollection = objectSearcher.Get();

            foreach (ManagementObject managementObject in objectCollection)
                yield return new Header_Extension_TypeGroup
                {
		 ...
		 KernelEventVersion = (uint) (managementObject.Properties["KernelEventVersion"]?.Value ?? default(uint))
                };
        }
    }
}