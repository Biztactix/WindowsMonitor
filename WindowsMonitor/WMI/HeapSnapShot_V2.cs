...
namespace WindowsMonitor.WMI
{
    /// <summary>
    /// </summary>
    public sealed class HeapSnapShot_V2
    {
        ...
        public static IEnumerable<HeapSnapShot_V2> Retrieve(string remote, string username, string password)
        {
        ...
        public static IEnumerable<HeapSnapShot_V2> Retrieve()
        {
            var managementScope = new ManagementScope(new ManagementPath("root\\wmi"));
            return Retrieve(managementScope);
        }

        public static IEnumerable<HeapSnapShot_V2> Retrieve(ManagementScope managementScope)
        {
            var objectQuery = new ObjectQuery("SELECT * FROM HeapSnapShot_V2");
            var objectSearcher = new ManagementObjectSearcher(managementScope, objectQuery);
            var objectCollection = objectSearcher.Get();

            foreach (ManagementObject managementObject in objectCollection)
                yield return new HeapSnapShot_V2
                {
