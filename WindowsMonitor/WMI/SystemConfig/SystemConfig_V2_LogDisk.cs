...
namespace WindowsMonitor.WMI
{
    /// <summary>
    /// </summary>
    public sealed class SystemConfig_V2_LogDisk
    {
		...
		public uint Pad3 { get; private set; }
        ...
        public static IEnumerable<SystemConfig_V2_LogDisk> Retrieve(string remote, string username, string password)
        {
        ...
        public static IEnumerable<SystemConfig_V2_LogDisk> Retrieve()
        {
            var managementScope = new ManagementScope(new ManagementPath("root\\wmi"));
            return Retrieve(managementScope);
        }

        public static IEnumerable<SystemConfig_V2_LogDisk> Retrieve(ManagementScope managementScope)
        {
            var objectQuery = new ObjectQuery("SELECT * FROM SystemConfig_V2_LogDisk");
            var objectSearcher = new ManagementObjectSearcher(managementScope, objectQuery);
            var objectCollection = objectSearcher.Get();

            foreach (ManagementObject managementObject in objectCollection)
                yield return new SystemConfig_V2_LogDisk
                {
		 ...
		 Pad3 = (uint) (managementObject.Properties["Pad3"]?.Value ?? default(uint)),
