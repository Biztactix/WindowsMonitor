...
namespace WindowsMonitor.WMI
{
    /// <summary>
    /// </summary>
    public sealed class Process_Defunct_TypeGroup1
    {
		public string ApplicationId { get; private set; }
		public string CommandLine { get; private set; }
		public uint DirectoryTableBase { get; private set; }
		public int ExitStatus { get; private set; }
		public ulong ExitTime { get; private set; }
        ...
        public static IEnumerable<Process_Defunct_TypeGroup1> Retrieve(string remote, string username, string password)
        {
        ...
        public static IEnumerable<Process_Defunct_TypeGroup1> Retrieve()
        {
            var managementScope = new ManagementScope(new ManagementPath("root\\wmi"));
            return Retrieve(managementScope);
        }

        public static IEnumerable<Process_Defunct_TypeGroup1> Retrieve(ManagementScope managementScope)
        {
            var objectQuery = new ObjectQuery("SELECT * FROM Process_Defunct_TypeGroup1");
            var objectSearcher = new ManagementObjectSearcher(managementScope, objectQuery);
            var objectCollection = objectSearcher.Get();

            foreach (ManagementObject managementObject in objectCollection)
                yield return new Process_Defunct_TypeGroup1
                {
		 ...
		 ExitTime = (ulong) (managementObject.Properties["ExitTime"]?.Value ?? default(ulong)),
