...
namespace WindowsMonitor.WMI
{
    /// <summary>
    /// </summary>
    public sealed class CSwitch_V3
    {
		...
		public sbyte ThreadFlags { get; private set; }

        public static IEnumerable<CSwitch_V3> Retrieve(string remote, string username, string password)
        {
        ...
        public static IEnumerable<CSwitch_V3> Retrieve()
        {
            var managementScope = new ManagementScope(new ManagementPath("root\\wmi"));
            return Retrieve(managementScope);
        }

        public static IEnumerable<CSwitch_V3> Retrieve(ManagementScope managementScope)
        {
            var objectQuery = new ObjectQuery("SELECT * FROM CSwitch_V3");
            var objectSearcher = new ManagementObjectSearcher(managementScope, objectQuery);
            var objectCollection = objectSearcher.Get();

            foreach (ManagementObject managementObject in objectCollection)
                yield return new CSwitch_V3
                {
		 ...
		 ThreadFlags = (sbyte) (managementObject.Properties["ThreadFlags"]?.Value ?? default(sbyte))
                };
        }
    }
}