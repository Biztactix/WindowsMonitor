...
namespace WindowsMonitor.WMI
{
    /// <summary>
    /// </summary>
    public sealed class Thread_TypeGroup1
    {
        ...
        public static IEnumerable<Thread_TypeGroup1> Retrieve(string remote, string username, string password)
        {
        ...
        public static IEnumerable<Thread_TypeGroup1> Retrieve()
        {
            var managementScope = new ManagementScope(new ManagementPath("root\\wmi"));
            return Retrieve(managementScope);
        }

        public static IEnumerable<Thread_TypeGroup1> Retrieve(ManagementScope managementScope)
        {
            var objectQuery = new ObjectQuery("SELECT * FROM Thread_TypeGroup1");
            var objectSearcher = new ManagementObjectSearcher(managementScope, objectQuery);
            var objectCollection = objectSearcher.Get();

            foreach (ManagementObject managementObject in objectCollection)
                yield return new Thread_TypeGroup1
                {
