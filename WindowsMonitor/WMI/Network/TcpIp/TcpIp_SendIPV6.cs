...
namespace WindowsMonitor.WMI
{
    /// <summary>
    /// </summary>
    public sealed class TcpIp_SendIPV6
    {
        ...
        public static IEnumerable<TcpIp_SendIPV6> Retrieve(string remote, string username, string password)
        {
        ...
        public static IEnumerable<TcpIp_SendIPV6> Retrieve()
        {
            var managementScope = new ManagementScope(new ManagementPath("root\\wmi"));
            return Retrieve(managementScope);
        }

        public static IEnumerable<TcpIp_SendIPV6> Retrieve(ManagementScope managementScope)
        {
            var objectQuery = new ObjectQuery("SELECT * FROM TcpIp_SendIPV6");
            var objectSearcher = new ManagementObjectSearcher(managementScope, objectQuery);
            var objectCollection = objectSearcher.Get();

            foreach (ManagementObject managementObject in objectCollection)
                yield return new TcpIp_SendIPV6
                {
