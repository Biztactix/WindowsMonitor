...
namespace WindowsMonitor.WMI
{
    /// <summary>
    /// </summary>
    public sealed class HvCreateIntMapping
    {
        ...
        public static IEnumerable<HvCreateIntMapping> Retrieve(string remote, string username, string password)
        {
        ...
        public static IEnumerable<HvCreateIntMapping> Retrieve()
        {
            var managementScope = new ManagementScope(new ManagementPath("root\\wmi"));
            return Retrieve(managementScope);
        }

        public static IEnumerable<HvCreateIntMapping> Retrieve(ManagementScope managementScope)
        {
            var objectQuery = new ObjectQuery("SELECT * FROM HvCreateIntMapping");
            var objectSearcher = new ManagementObjectSearcher(managementScope, objectQuery);
            var objectCollection = objectSearcher.Get();

            foreach (ManagementObject managementObject in objectCollection)
                yield return new HvCreateIntMapping
                {
