...
namespace WindowsMonitor.WMI
{
    /// <summary>
    /// </summary>
    public sealed class DiskIo_TypeGroup1
    {
        ...
        public static IEnumerable<DiskIo_TypeGroup1> Retrieve(string remote, string username, string password)
        {
        ...
        public static IEnumerable<DiskIo_TypeGroup1> Retrieve()
        {
            var managementScope = new ManagementScope(new ManagementPath("root\\wmi"));
            return Retrieve(managementScope);
        }

        public static IEnumerable<DiskIo_TypeGroup1> Retrieve(ManagementScope managementScope)
        {
            var objectQuery = new ObjectQuery("SELECT * FROM DiskIo_TypeGroup1");
            var objectSearcher = new ManagementObjectSearcher(managementScope, objectQuery);
            var objectCollection = objectSearcher.Get();

            foreach (ManagementObject managementObject in objectCollection)
                yield return new DiskIo_TypeGroup1
                {
