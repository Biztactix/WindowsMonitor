...
namespace WindowsMonitor.CIM
{
    /// <summary>
    /// </summary>
    public sealed class ProtectedSpaceExtent
    {
		...
		public ulong UserDataStripeDepth { get; private set; }

        public static IEnumerable<ProtectedSpaceExtent> Retrieve(string remote, string username, string password)
        {
        ...
        public static IEnumerable<ProtectedSpaceExtent> Retrieve()
        {
            var managementScope = new ManagementScope(new ManagementPath("root\\cimv2"));
            return Retrieve(managementScope);
        }

        public static IEnumerable<ProtectedSpaceExtent> Retrieve(ManagementScope managementScope)
        {
            var objectQuery = new ObjectQuery("SELECT * FROM CIM_ProtectedSpaceExtent");
            var objectSearcher = new ManagementObjectSearcher(managementScope, objectQuery);
            var objectCollection = objectSearcher.Get();

            foreach (ManagementObject managementObject in objectCollection)
                yield return new ProtectedSpaceExtent
                {
		 ...
		 UserDataStripeDepth = (ulong) (managementObject.Properties["UserDataStripeDepth"]?.Value ?? default(ulong))
                };
        }
    }
}