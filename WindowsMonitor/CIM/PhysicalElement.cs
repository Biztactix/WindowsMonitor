...
namespace WindowsMonitor.CIM
{
    /// <summary>
    /// </summary>
    public sealed class PhysicalElement
    {
        ...
        public static IEnumerable<PhysicalElement> Retrieve(string remote, string username, string password)
        {
        ...
        public static IEnumerable<PhysicalElement> Retrieve()
        {
            var managementScope = new ManagementScope(new ManagementPath("root\\cimv2"));
            return Retrieve(managementScope);
        }

        public static IEnumerable<PhysicalElement> Retrieve(ManagementScope managementScope)
        {
            var objectQuery = new ObjectQuery("SELECT * FROM CIM_PhysicalElement");
            var objectSearcher = new ManagementObjectSearcher(managementScope, objectQuery);
            var objectCollection = objectSearcher.Get();

            foreach (ManagementObject managementObject in objectCollection)
                yield return new PhysicalElement
                {
