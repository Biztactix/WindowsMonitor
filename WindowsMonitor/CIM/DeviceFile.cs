...
namespace WindowsMonitor.CIM
{
    ...
    public sealed class DeviceFile
    {
        ...
        public static IEnumerable<DeviceFile> Retrieve(string remote, string username, string password)
        {
        ...
        public static IEnumerable<DeviceFile> Retrieve()
        {
            var managementScope = new ManagementScope(new ManagementPath("root\\cimv2"));
            return Retrieve(managementScope);
        }

        public static IEnumerable<DeviceFile> Retrieve(ManagementScope managementScope)
        {
            var objectQuery = new ObjectQuery("SELECT * FROM CIM_DeviceFile");
            var objectSearcher = new ManagementObjectSearcher(managementScope, objectQuery);
            var objectCollection = objectSearcher.Get();

            foreach (ManagementObject managementObject in objectCollection)
                yield return new DeviceFile
                {
