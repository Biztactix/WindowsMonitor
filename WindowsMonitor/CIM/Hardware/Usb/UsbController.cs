...
namespace WindowsMonitor.CIM.Hardware.Usb
{
    /// <summary>
    /// </summary>
    public sealed class UsbController
    {
        ...
        public static IEnumerable<UsbController> Retrieve(string remote, string username, string password)
        {
        ...
        public static IEnumerable<UsbController> Retrieve()
        {
            var managementScope = new ManagementScope(new ManagementPath("root\\cimv2"));
            return Retrieve(managementScope);
        }

        public static IEnumerable<UsbController> Retrieve(ManagementScope managementScope)
        {
            var objectQuery = new ObjectQuery("SELECT * FROM CIM_USBController");
            var objectSearcher = new ManagementObjectSearcher(managementScope, objectQuery);
            var objectCollection = objectSearcher.Get();

            foreach (ManagementObject managementObject in objectCollection)
                yield return new UsbController
                {
