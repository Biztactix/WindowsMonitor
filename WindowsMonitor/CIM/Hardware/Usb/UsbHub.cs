...
namespace WindowsMonitor.CIM.Hardware.Usb
{
    ...
    public sealed class UsbHub
    {
        ...
        public static IEnumerable<UsbHub> Retrieve(string remote, string username, string password)
        {
        ...
        public static IEnumerable<UsbHub> Retrieve()
        {
            var managementScope = new ManagementScope(new ManagementPath("root\\cimv2"));
            return Retrieve(managementScope);
        }

        public static IEnumerable<UsbHub> Retrieve(ManagementScope managementScope)
        {
            var objectQuery = new ObjectQuery("SELECT * FROM CIM_USBHub");
            var objectSearcher = new ManagementObjectSearcher(managementScope, objectQuery);
            var objectCollection = objectSearcher.Get();

            foreach (ManagementObject managementObject in objectCollection)
                yield return new UsbHub
                {
