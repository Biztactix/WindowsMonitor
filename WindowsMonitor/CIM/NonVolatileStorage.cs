...
namespace WindowsMonitor.CIM
{
    /// <summary>
    /// </summary>
    public sealed class NonVolatileStorage
    {
        ...
        public static IEnumerable<NonVolatileStorage> Retrieve(string remote, string username, string password)
        {
        ...
        public static IEnumerable<NonVolatileStorage> Retrieve()
        {
            var managementScope = new ManagementScope(new ManagementPath("root\\cimv2"));
            return Retrieve(managementScope);
        }

        public static IEnumerable<NonVolatileStorage> Retrieve(ManagementScope managementScope)
        {
            var objectQuery = new ObjectQuery("SELECT * FROM CIM_NonVolatileStorage");
            var objectSearcher = new ManagementObjectSearcher(managementScope, objectQuery);
            var objectCollection = objectSearcher.Get();

            foreach (ManagementObject managementObject in objectCollection)
                yield return new NonVolatileStorage
                {
		 ...
		 IsWriteable = (bool) (managementObject.Properties["IsWriteable"]?.Value ?? default(bool)),
