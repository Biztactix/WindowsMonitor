...
namespace WindowsMonitor.CIM
{
    /// <summary>
    /// </summary>
    public sealed class MediaAccessDevice
    {
        ...
        public static IEnumerable<MediaAccessDevice> Retrieve(string remote, string username, string password)
        {
        ...
        public static IEnumerable<MediaAccessDevice> Retrieve()
        {
            var managementScope = new ManagementScope(new ManagementPath("root\\cimv2"));
            return Retrieve(managementScope);
        }

        public static IEnumerable<MediaAccessDevice> Retrieve(ManagementScope managementScope)
        {
            var objectQuery = new ObjectQuery("SELECT * FROM CIM_MediaAccessDevice");
            var objectSearcher = new ManagementObjectSearcher(managementScope, objectQuery);
            var objectCollection = objectSearcher.Get();

            foreach (ManagementObject managementObject in objectCollection)
                yield return new MediaAccessDevice
                {
