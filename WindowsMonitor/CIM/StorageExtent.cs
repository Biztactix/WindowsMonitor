...
namespace WindowsMonitor.CIM
{
    /// <summary>
    /// </summary>
    public sealed class StorageExtent
    {
        ...
        public static IEnumerable<StorageExtent> Retrieve(string remote, string username, string password)
        {
        ...
        public static IEnumerable<StorageExtent> Retrieve()
        {
            var managementScope = new ManagementScope(new ManagementPath("root\\cimv2"));
            return Retrieve(managementScope);
        }

        public static IEnumerable<StorageExtent> Retrieve(ManagementScope managementScope)
        {
            var objectQuery = new ObjectQuery("SELECT * FROM CIM_StorageExtent");
            var objectSearcher = new ManagementObjectSearcher(managementScope, objectQuery);
            var objectCollection = objectSearcher.Get();

            foreach (ManagementObject managementObject in objectCollection)
                yield return new StorageExtent
                {
