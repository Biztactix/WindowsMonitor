...
namespace WindowsMonitor.CIM
{
    ...
    public sealed class StorageVolume
    {
        ...
        public static IEnumerable<StorageVolume> Retrieve(string remote, string username, string password)
        {
        ...
        public static IEnumerable<StorageVolume> Retrieve()
        {
            var managementScope = new ManagementScope(new ManagementPath("root\\cimv2"));
            return Retrieve(managementScope);
        }

        public static IEnumerable<StorageVolume> Retrieve(ManagementScope managementScope)
        {
            var objectQuery = new ObjectQuery("SELECT * FROM CIM_StorageVolume");
            var objectSearcher = new ManagementObjectSearcher(managementScope, objectQuery);
            var objectCollection = objectSearcher.Get();

            foreach (ManagementObject managementObject in objectCollection)
                yield return new StorageVolume
                {
