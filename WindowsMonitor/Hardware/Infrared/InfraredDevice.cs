...
namespace WindowsMonitor.Hardware.Infrared
{
    /// <summary>
    /// </summary>
    public sealed class InfraredDevice
    {
        ...
        public static IEnumerable<InfraredDevice> Retrieve(string remote, string username, string password)
        {
        ...
        public static IEnumerable<InfraredDevice> Retrieve()
        {
            var managementScope = new ManagementScope(new ManagementPath("root\\cimv2"));
            return Retrieve(managementScope);
        }

        public static IEnumerable<InfraredDevice> Retrieve(ManagementScope managementScope)
        {
            var objectQuery = new ObjectQuery("SELECT * FROM Win32_InfraredDevice");
            var objectSearcher = new ManagementObjectSearcher(managementScope, objectQuery);
            var objectCollection = objectSearcher.Get();

            foreach (ManagementObject managementObject in objectCollection)
                yield return new InfraredDevice
                {
