...
namespace WindowsMonitor.CIM.Hardware.Sensors
{
    /// <summary>
    /// </summary>
    public sealed class NumericSensor
    {
        ...
        public static IEnumerable<NumericSensor> Retrieve(string remote, string username, string password)
        {
        ...
        public static IEnumerable<NumericSensor> Retrieve()
        {
            var managementScope = new ManagementScope(new ManagementPath("root\\cimv2"));
            return Retrieve(managementScope);
        }

        public static IEnumerable<NumericSensor> Retrieve(ManagementScope managementScope)
        {
            var objectQuery = new ObjectQuery("SELECT * FROM CIM_NumericSensor");
            var objectSearcher = new ManagementObjectSearcher(managementScope, objectQuery);
            var objectCollection = objectSearcher.Get();

            foreach (ManagementObject managementObject in objectCollection)
                yield return new NumericSensor
                {
                    ...
                    LowerThresholdCritical =
                        (int) (managementObject.Properties["LowerThresholdCritical"]?.Value ?? default(int)),
                    LowerThresholdFatal =
                        (int) (managementObject.Properties["LowerThresholdFatal"]?.Value ?? default(int)),
                    LowerThresholdNonCritical =
                        (int) (managementObject.Properties["LowerThresholdNonCritical"]?.Value ?? default(int)),
                    ...
                    UpperThresholdCritical =
                        (int) (managementObject.Properties["UpperThresholdCritical"]?.Value ?? default(int)),
                    UpperThresholdFatal =
                        (int) (managementObject.Properties["UpperThresholdFatal"]?.Value ?? default(int)),
                    UpperThresholdNonCritical =
                        (int) (managementObject.Properties["UpperThresholdNonCritical"]?.Value ?? default(int))
                };
        }
    }
}