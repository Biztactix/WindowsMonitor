...
namespace WindowsMonitor.Hardware
{
    ...
    public sealed class Chassis
    {
        ...
        public static IEnumerable<Chassis> Retrieve(string remote, string username, string password)
        {
        ...
        public static IEnumerable<Chassis> Retrieve()
        {
            var managementScope = new ManagementScope(new ManagementPath("root\\cimv2"));
            return Retrieve(managementScope);
        }

        public static IEnumerable<Chassis> Retrieve(ManagementScope managementScope)
        {
            var objectQuery = new ObjectQuery("SELECT * FROM CIM_Chassis");
            var objectSearcher = new ManagementObjectSearcher(managementScope, objectQuery);
            var objectCollection = objectSearcher.Get();

            foreach (ManagementObject managementObject in objectCollection)
                yield return new Chassis
                {
                    ...
                    CurrentRequiredOrProduced =
                         (managementObject.Properties["CurrentRequiredOrProduced"]?.Value?.ToString()),
                    ...
                    NumberOfPowerCords =
                        (ushort) (managementObject.Properties["NumberOfPowerCords"]?.Value ?? default(ushort)),
                    ...
                    ServiceDescriptions =
                        (string[]) (managementObject.Properties["ServiceDescriptions"]?.Value ?? new string[0]),
                    ServicePhilosophy =
                        (ushort[]) (managementObject.Properties["ServicePhilosophy"]?.Value ?? new ushort[0]),
                    Sku = (string) managementObject.Properties["SKU"]?.Value,
                    Status = (string) managementObject.Properties["Status"]?.Value,
                    Tag = (string) managementObject.Properties["Tag"]?.Value,
                    TypeDescriptions =
                        (string[]) (managementObject.Properties["TypeDescriptions"]?.Value ?? new string[0]),
