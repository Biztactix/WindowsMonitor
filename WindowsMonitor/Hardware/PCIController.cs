...
namespace WindowsMonitor.CIM.Hardware
{
    ...
    public sealed class PciController
    {
        ...
        public static IEnumerable<PciController> Retrieve(string remote, string username, string password)
        {
        ...
        public static IEnumerable<PciController> Retrieve()
        {
            var managementScope = new ManagementScope(new ManagementPath("root\\cimv2"));
            return Retrieve(managementScope);
        }

        public static IEnumerable<PciController> Retrieve(ManagementScope managementScope)
        {
            var objectQuery = new ObjectQuery("SELECT * FROM CIM_PCIController");
            var objectSearcher = new ManagementObjectSearcher(managementScope, objectQuery);
            var objectCollection = objectSearcher.Get();

            foreach (ManagementObject managementObject in objectCollection)
                yield return new PciController
                {
