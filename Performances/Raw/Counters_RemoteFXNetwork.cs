...
namespace ComputerManagment
{
    /// <summary>
    /// </summary>
    public sealed class PerfRawData_Counters_RemoteFXNetwork
    {
        ...
        public static PerfRawData_Counters_RemoteFXNetwork[] Retrieve(string remote, string username, string password)
        {
        ...
        public static PerfRawData_Counters_RemoteFXNetwork[] Retrieve()
        {
            var managementScope = new ManagementScope(new ManagementPath("root\\cimv2"));
            return Retrieve(managementScope);
        }

        public static PerfRawData_Counters_RemoteFXNetwork[] Retrieve(ManagementScope managementScope)
        {
            var objectQuery = new ObjectQuery("SELECT * FROM Win32_PerfRawData_Counters_RemoteFXNetwork");
            var objectSearcher = new ManagementObjectSearcher(managementScope, objectQuery);
            var objectCollection = objectSearcher.Get();

            var list = new List<PerfRawData_Counters_RemoteFXNetwork>();

            foreach (ManagementObject managementObject in objectCollection)
                list.Add(new PerfRawData_Counters_RemoteFXNetwork
                {
                    ...
                    FECRate_Base = (uint) managementObject.Properties["FECRate_Base"].Value,
                    Frequency_Object = (ulong) managementObject.Properties["Frequency_Object"].Value,
                    Frequency_PerfTime = (ulong) managementObject.Properties["Frequency_PerfTime"].Value,
                    Frequency_Sys100NS = (ulong) managementObject.Properties["Frequency_Sys100NS"].Value,
                    LossRate = (uint) managementObject.Properties["LossRate"].Value,
                    LossRate_Base = (uint) managementObject.Properties["LossRate_Base"].Value,
                    Name = (string) managementObject.Properties["Name"].Value,
                    RetransmissionRate = (uint) managementObject.Properties["RetransmissionRate"].Value,
                    RetransmissionRate_Base = (uint) managementObject.Properties["RetransmissionRate_Base"].Value,
