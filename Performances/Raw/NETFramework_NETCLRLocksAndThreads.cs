...
namespace ComputerManagment
{
    /// <summary>
    /// </summary>
    public sealed class PerfRawData_NETFramework_NETCLRLocksAndThreads
    {
        ...
        public static PerfRawData_NETFramework_NETCLRLocksAndThreads[] Retrieve(string remote, string username,
            string password)
        {
        ...
        public static PerfRawData_NETFramework_NETCLRLocksAndThreads[] Retrieve()
        {
            var managementScope = new ManagementScope(new ManagementPath("root\\cimv2"));
            return Retrieve(managementScope);
        }

        public static PerfRawData_NETFramework_NETCLRLocksAndThreads[] Retrieve(ManagementScope managementScope)
        {
            var objectQuery = new ObjectQuery("SELECT * FROM Win32_PerfRawData_NETFramework_NETCLRLocksAndThreads");
            var objectSearcher = new ManagementObjectSearcher(managementScope, objectQuery);
            var objectCollection = objectSearcher.Get();

            var list = new List<PerfRawData_NETFramework_NETCLRLocksAndThreads>();

            foreach (ManagementObject managementObject in objectCollection)
                list.Add(new PerfRawData_NETFramework_NETCLRLocksAndThreads
                {
