...
namespace ComputerManagment
{
    /// <summary>
    /// </summary>
    public sealed class PerfRawData_XTPDatabaseEngine_XTPTransactions
    {
        ...
        public static PerfRawData_XTPDatabaseEngine_XTPTransactions[] Retrieve(string remote, string username,
            string password)
        {
        ...
        public static PerfRawData_XTPDatabaseEngine_XTPTransactions[] Retrieve()
        {
            var managementScope = new ManagementScope(new ManagementPath("root\\cimv2"));
            return Retrieve(managementScope);
        }

        public static PerfRawData_XTPDatabaseEngine_XTPTransactions[] Retrieve(ManagementScope managementScope)
        {
            var objectQuery = new ObjectQuery("SELECT * FROM Win32_PerfRawData_XTPDatabaseEngine_XTPTransactions");
            var objectSearcher = new ManagementObjectSearcher(managementScope, objectQuery);
            var objectCollection = objectSearcher.Get();

            var list = new List<PerfRawData_XTPDatabaseEngine_XTPTransactions>();

            foreach (ManagementObject managementObject in objectCollection)
                list.Add(new PerfRawData_XTPDatabaseEngine_XTPTransactions
                {
