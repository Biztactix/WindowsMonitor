...
namespace ComputerManagment
{
    /// <summary>
    /// </summary>
    public sealed class PerfRawData_APPPOOLCountersProvider_APPPOOLWAS
    {
        ...
        public static PerfRawData_APPPOOLCountersProvider_APPPOOLWAS[] Retrieve(string remote, string username,
            string password)
        {
        ...
        public static PerfRawData_APPPOOLCountersProvider_APPPOOLWAS[] Retrieve()
        {
            var managementScope = new ManagementScope(new ManagementPath("root\\cimv2"));
            return Retrieve(managementScope);
        }

        public static PerfRawData_APPPOOLCountersProvider_APPPOOLWAS[] Retrieve(ManagementScope managementScope)
        {
            var objectQuery = new ObjectQuery("SELECT * FROM Win32_PerfRawData_APPPOOLCountersProvider_APPPOOLWAS");
            var objectSearcher = new ManagementObjectSearcher(managementScope, objectQuery);
            var objectCollection = objectSearcher.Get();

            var list = new List<PerfRawData_APPPOOLCountersProvider_APPPOOLWAS>();

            foreach (ManagementObject managementObject in objectCollection)
                list.Add(new PerfRawData_APPPOOLCountersProvider_APPPOOLWAS
                {
