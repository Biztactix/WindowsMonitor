...
namespace ComputerManagment
{
    /// <summary>
    /// </summary>
    public sealed class PerfRawData_HvStats_HyperVHypervisor
    {
        ...
        public static PerfRawData_HvStats_HyperVHypervisor[] Retrieve(string remote, string username, string password)
        {
        ...
        public static PerfRawData_HvStats_HyperVHypervisor[] Retrieve()
        {
            var managementScope = new ManagementScope(new ManagementPath("root\\cimv2"));
            return Retrieve(managementScope);
        }

        public static PerfRawData_HvStats_HyperVHypervisor[] Retrieve(ManagementScope managementScope)
        {
            var objectQuery = new ObjectQuery("SELECT * FROM Win32_PerfRawData_HvStats_HyperVHypervisor");
            var objectSearcher = new ManagementObjectSearcher(managementScope, objectQuery);
            var objectCollection = objectSearcher.Get();

            var list = new List<PerfRawData_HvStats_HyperVHypervisor>();

            foreach (ManagementObject managementObject in objectCollection)
                list.Add(new PerfRawData_HvStats_HyperVHypervisor
                {
