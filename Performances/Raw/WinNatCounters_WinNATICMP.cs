...
namespace ComputerManagment
{
    /// <summary>
    /// </summary>
    public sealed class PerfRawData_WinNatCounters_WinNATICMP
    {
        ...
        public static PerfRawData_WinNatCounters_WinNATICMP[] Retrieve(string remote, string username, string password)
        {
        ...
        public static PerfRawData_WinNatCounters_WinNATICMP[] Retrieve()
        {
            var managementScope = new ManagementScope(new ManagementPath("root\\cimv2"));
            return Retrieve(managementScope);
        }

        public static PerfRawData_WinNatCounters_WinNATICMP[] Retrieve(ManagementScope managementScope)
        {
            var objectQuery = new ObjectQuery("SELECT * FROM Win32_PerfRawData_WinNatCounters_WinNATICMP");
            var objectSearcher = new ManagementObjectSearcher(managementScope, objectQuery);
            var objectCollection = objectSearcher.Get();

            var list = new List<PerfRawData_WinNatCounters_WinNATICMP>();

            foreach (ManagementObject managementObject in objectCollection)
                list.Add(new PerfRawData_WinNatCounters_WinNATICMP
                {
