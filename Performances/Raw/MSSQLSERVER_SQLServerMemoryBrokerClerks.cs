...
namespace ComputerManagment
{
    /// <summary>
    /// </summary>
    public sealed class PerfRawData_MSSQLSERVER_SQLServerMemoryBrokerClerks
    {
        ...
        public static PerfRawData_MSSQLSERVER_SQLServerMemoryBrokerClerks[] Retrieve(string remote, string username,
            string password)
        {
        ...
        public static PerfRawData_MSSQLSERVER_SQLServerMemoryBrokerClerks[] Retrieve()
        {
            var managementScope = new ManagementScope(new ManagementPath("root\\cimv2"));
            return Retrieve(managementScope);
        }

        public static PerfRawData_MSSQLSERVER_SQLServerMemoryBrokerClerks[] Retrieve(ManagementScope managementScope)
        {
            var objectQuery =
                new ObjectQuery("SELECT * FROM Win32_PerfRawData_MSSQLSERVER_SQLServerMemoryBrokerClerks");
            var objectSearcher = new ManagementObjectSearcher(managementScope, objectQuery);
            var objectCollection = objectSearcher.Get();

            var list = new List<PerfRawData_MSSQLSERVER_SQLServerMemoryBrokerClerks>();

            foreach (ManagementObject managementObject in objectCollection)
                list.Add(new PerfRawData_MSSQLSERVER_SQLServerMemoryBrokerClerks
                {
