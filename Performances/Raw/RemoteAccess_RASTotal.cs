...
namespace ComputerManagment
{
    /// <summary>
    /// </summary>
    public sealed class PerfRawData_RemoteAccess_RASTotal
    {
        ...
        public static PerfRawData_RemoteAccess_RASTotal[] Retrieve(string remote, string username, string password)
        {
        ...
        public static PerfRawData_RemoteAccess_RASTotal[] Retrieve()
        {
            var managementScope = new ManagementScope(new ManagementPath("root\\cimv2"));
            return Retrieve(managementScope);
        }

        public static PerfRawData_RemoteAccess_RASTotal[] Retrieve(ManagementScope managementScope)
        {
            var objectQuery = new ObjectQuery("SELECT * FROM Win32_PerfRawData_RemoteAccess_RASTotal");
            var objectSearcher = new ManagementObjectSearcher(managementScope, objectQuery);
            var objectCollection = objectSearcher.Get();

            var list = new List<PerfRawData_RemoteAccess_RASTotal>();

            foreach (ManagementObject managementObject in objectCollection)
                list.Add(new PerfRawData_RemoteAccess_RASTotal
                {
