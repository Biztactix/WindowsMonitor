...
namespace ComputerManagment
{
    /// <summary>
    /// </summary>
    public sealed class PerfRawData_MSSQLSERVER_SQLServerColumnstore
    {
        ...
        public static PerfRawData_MSSQLSERVER_SQLServerColumnstore[] Retrieve(string remote, string username,
            string password)
        {
        ...
        public static PerfRawData_MSSQLSERVER_SQLServerColumnstore[] Retrieve()
        {
            var managementScope = new ManagementScope(new ManagementPath("root\\cimv2"));
            return Retrieve(managementScope);
        }

        public static PerfRawData_MSSQLSERVER_SQLServerColumnstore[] Retrieve(ManagementScope managementScope)
        {
            var objectQuery = new ObjectQuery("SELECT * FROM Win32_PerfRawData_MSSQLSERVER_SQLServerColumnstore");
            var objectSearcher = new ManagementObjectSearcher(managementScope, objectQuery);
            var objectCollection = objectSearcher.Get();

            var list = new List<PerfRawData_MSSQLSERVER_SQLServerColumnstore>();

            foreach (ManagementObject managementObject in objectCollection)
                list.Add(new PerfRawData_MSSQLSERVER_SQLServerColumnstore
                {
                    ...
                    SegmentCacheHitRatio_Base = (ulong) managementObject.Properties["SegmentCacheHitRatio_Base"].Value,
