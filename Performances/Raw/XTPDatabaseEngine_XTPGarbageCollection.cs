...
namespace ComputerManagment
{
    /// <summary>
    /// </summary>
    public sealed class PerfRawData_XTPDatabaseEngine_XTPGarbageCollection
    {
        ...
        public static PerfRawData_XTPDatabaseEngine_XTPGarbageCollection[] Retrieve(string remote, string username,
            string password)
        {
        ...
        public static PerfRawData_XTPDatabaseEngine_XTPGarbageCollection[] Retrieve()
        {
            var managementScope = new ManagementScope(new ManagementPath("root\\cimv2"));
            return Retrieve(managementScope);
        }

        public static PerfRawData_XTPDatabaseEngine_XTPGarbageCollection[] Retrieve(ManagementScope managementScope)
        {
            var objectQuery = new ObjectQuery("SELECT * FROM Win32_PerfRawData_XTPDatabaseEngine_XTPGarbageCollection");
            var objectSearcher = new ManagementObjectSearcher(managementScope, objectQuery);
            var objectCollection = objectSearcher.Get();

            var list = new List<PerfRawData_XTPDatabaseEngine_XTPGarbageCollection>();

            foreach (ManagementObject managementObject in objectCollection)
                list.Add(new PerfRawData_XTPDatabaseEngine_XTPGarbageCollection
                {
