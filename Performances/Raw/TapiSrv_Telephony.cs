...
namespace ComputerManagment
{
    /// <summary>
    /// </summary>
    public sealed class PerfRawData_TapiSrv_Telephony
    {
        ...
        public static PerfRawData_TapiSrv_Telephony[] Retrieve(string remote, string username, string password)
        {
        ...
        public static PerfRawData_TapiSrv_Telephony[] Retrieve()
        {
            var managementScope = new ManagementScope(new ManagementPath("root\\cimv2"));
            return Retrieve(managementScope);
        }

        public static PerfRawData_TapiSrv_Telephony[] Retrieve(ManagementScope managementScope)
        {
            var objectQuery = new ObjectQuery("SELECT * FROM Win32_PerfRawData_TapiSrv_Telephony");
            var objectSearcher = new ManagementObjectSearcher(managementScope, objectQuery);
            var objectCollection = objectSearcher.Get();

            var list = new List<PerfRawData_TapiSrv_Telephony>();

            foreach (ManagementObject managementObject in objectCollection)
                list.Add(new PerfRawData_TapiSrv_Telephony
                {
