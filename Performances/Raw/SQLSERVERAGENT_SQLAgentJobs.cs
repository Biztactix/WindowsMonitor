...
namespace ComputerManagment
{
    /// <summary>
    /// </summary>
    public sealed class PerfRawData_SQLSERVERAGENT_SQLAgentJobs
    {
        ...
        public static PerfRawData_SQLSERVERAGENT_SQLAgentJobs[] Retrieve(string remote, string username,
            string password)
        {
        ...
        public static PerfRawData_SQLSERVERAGENT_SQLAgentJobs[] Retrieve()
        {
            var managementScope = new ManagementScope(new ManagementPath("root\\cimv2"));
            return Retrieve(managementScope);
        }

        public static PerfRawData_SQLSERVERAGENT_SQLAgentJobs[] Retrieve(ManagementScope managementScope)
        {
            var objectQuery = new ObjectQuery("SELECT * FROM Win32_PerfRawData_SQLSERVERAGENT_SQLAgentJobs");
            var objectSearcher = new ManagementObjectSearcher(managementScope, objectQuery);
            var objectCollection = objectSearcher.Get();

            var list = new List<PerfRawData_SQLSERVERAGENT_SQLAgentJobs>();

            foreach (ManagementObject managementObject in objectCollection)
                list.Add(new PerfRawData_SQLSERVERAGENT_SQLAgentJobs
                {
