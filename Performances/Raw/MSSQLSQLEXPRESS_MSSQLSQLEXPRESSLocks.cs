...
namespace ComputerManagment
{
    /// <summary>
    /// </summary>
    public sealed class PerfRawData_MSSQLSQLEXPRESS_MSSQLSQLEXPRESSLocks
    {
        ...
        public static PerfRawData_MSSQLSQLEXPRESS_MSSQLSQLEXPRESSLocks[] Retrieve(string remote, string username,
            string password)
        {
        ...
        public static PerfRawData_MSSQLSQLEXPRESS_MSSQLSQLEXPRESSLocks[] Retrieve()
        {
            var managementScope = new ManagementScope(new ManagementPath("root\\cimv2"));
            return Retrieve(managementScope);
        }

        public static PerfRawData_MSSQLSQLEXPRESS_MSSQLSQLEXPRESSLocks[] Retrieve(ManagementScope managementScope)
        {
            var objectQuery = new ObjectQuery("SELECT * FROM Win32_PerfRawData_MSSQLSQLEXPRESS_MSSQLSQLEXPRESSLocks");
            var objectSearcher = new ManagementObjectSearcher(managementScope, objectQuery);
            var objectCollection = objectSearcher.Get();

            var list = new List<PerfRawData_MSSQLSQLEXPRESS_MSSQLSQLEXPRESSLocks>();

            foreach (ManagementObject managementObject in objectCollection)
                list.Add(new PerfRawData_MSSQLSQLEXPRESS_MSSQLSQLEXPRESSLocks
                {
                    AverageWaitTimems = (ulong) managementObject.Properties["AverageWaitTimems"].Value,
                    AverageWaitTimems_Base = (uint) managementObject.Properties["AverageWaitTimems_Base"].Value,
