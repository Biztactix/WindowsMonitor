...
namespace ComputerManagment
{
    /// <summary>
    /// </summary>
    public sealed class PerfRawData_PerfNet_ServerWorkQueues
    {
        ...
        public static PerfRawData_PerfNet_ServerWorkQueues[] Retrieve(string remote, string username, string password)
        {
        ...
        public static PerfRawData_PerfNet_ServerWorkQueues[] Retrieve()
        {
            var managementScope = new ManagementScope(new ManagementPath("root\\cimv2"));
            return Retrieve(managementScope);
        }

        public static PerfRawData_PerfNet_ServerWorkQueues[] Retrieve(ManagementScope managementScope)
        {
            var objectQuery = new ObjectQuery("SELECT * FROM Win32_PerfRawData_PerfNet_ServerWorkQueues");
            var objectSearcher = new ManagementObjectSearcher(managementScope, objectQuery);
            var objectCollection = objectSearcher.Get();

            var list = new List<PerfRawData_PerfNet_ServerWorkQueues>();

            foreach (ManagementObject managementObject in objectCollection)
                list.Add(new PerfRawData_PerfNet_ServerWorkQueues
                {
