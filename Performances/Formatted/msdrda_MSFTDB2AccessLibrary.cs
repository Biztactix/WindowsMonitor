...
namespace ComputerManagment
{
    /// <summary>
    /// </summary>
    public sealed class PerfFormattedData_msdrda_MSFTDB2AccessLibrary
    {
        ...
        public static PerfFormattedData_msdrda_MSFTDB2AccessLibrary[] Retrieve(string remote, string username,
            string password)
        {
        ...
        public static PerfFormattedData_msdrda_MSFTDB2AccessLibrary[] Retrieve()
        {
            var managementScope = new ManagementScope(new ManagementPath("root\\cimv2"));
            return Retrieve(managementScope);
        }

        public static PerfFormattedData_msdrda_MSFTDB2AccessLibrary[] Retrieve(ManagementScope managementScope)
        {
            var objectQuery = new ObjectQuery("SELECT * FROM Win32_PerfFormattedData_msdrda_MSFTDB2AccessLibrary");
            var objectSearcher = new ManagementObjectSearcher(managementScope, objectQuery);
            var objectCollection = objectSearcher.Get();

            var list = new List<PerfFormattedData_msdrda_MSFTDB2AccessLibrary>();

            foreach (ManagementObject managementObject in objectCollection)
                list.Add(new PerfFormattedData_msdrda_MSFTDB2AccessLibrary
                {
