...
namespace ComputerManagment
{
    /// <summary>
    /// </summary>
    public sealed class PerfFormattedData_RemoteAccess_RASTotal
    {
        ...
        public static PerfFormattedData_RemoteAccess_RASTotal[] Retrieve(string remote, string username,
            string password)
        {
        ...
        public static PerfFormattedData_RemoteAccess_RASTotal[] Retrieve()
        {
            var managementScope = new ManagementScope(new ManagementPath("root\\cimv2"));
            return Retrieve(managementScope);
        }

        public static PerfFormattedData_RemoteAccess_RASTotal[] Retrieve(ManagementScope managementScope)
        {
            var objectQuery = new ObjectQuery("SELECT * FROM Win32_PerfFormattedData_RemoteAccess_RASTotal");
            var objectSearcher = new ManagementObjectSearcher(managementScope, objectQuery);
            var objectCollection = objectSearcher.Get();

            var list = new List<PerfFormattedData_RemoteAccess_RASTotal>();

            foreach (ManagementObject managementObject in objectCollection)
                list.Add(new PerfFormattedData_RemoteAccess_RASTotal
                {
                    ...
                    TotalConnections = (uint) managementObject.Properties["TotalConnections"].Value,
