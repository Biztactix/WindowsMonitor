...
namespace ComputerManagment
{
    /// <summary>
    /// </summary>
    public sealed class PerfFormattedData_SQLSERVERAGENT_SQLAgentJobs
    {
        ...
        public static PerfFormattedData_SQLSERVERAGENT_SQLAgentJobs[] Retrieve(string remote, string username,
            string password)
        {
        ...
        public static PerfFormattedData_SQLSERVERAGENT_SQLAgentJobs[] Retrieve()
        {
            var managementScope = new ManagementScope(new ManagementPath("root\\cimv2"));
            return Retrieve(managementScope);
        }

        public static PerfFormattedData_SQLSERVERAGENT_SQLAgentJobs[] Retrieve(ManagementScope managementScope)
        {
            var objectQuery = new ObjectQuery("SELECT * FROM Win32_PerfFormattedData_SQLSERVERAGENT_SQLAgentJobs");
            var objectSearcher = new ManagementObjectSearcher(managementScope, objectQuery);
            var objectCollection = objectSearcher.Get();

            var list = new List<PerfFormattedData_SQLSERVERAGENT_SQLAgentJobs>();

            foreach (ManagementObject managementObject in objectCollection)
                list.Add(new PerfFormattedData_SQLSERVERAGENT_SQLAgentJobs
                {
