...
public String Version { get; private set; }
		...
		public static CreateFolderAction[] Retrieve(string remote, string username, string password)
		{
		...
		public static CreateFolderAction[] Retrieve()
		{
			var managementScope = new ManagementScope(new ManagementPath("root\\cimv2"));
			return Retrieve(managementScope);
		}

		public static CreateFolderAction[] Retrieve(ManagementScope managementScope)
		{
		    var objectQuery = new ObjectQuery("SELECT * FROM Win32_CreateFolderAction");
            var objectSearcher = new ManagementObjectSearcher(managementScope, objectQuery);
            var objectCollection = objectSearcher.Get();

			var list = new List<CreateFolderAction>();

            foreach (ManagementObject managementObject in objectCollection)
            {
				list.Add(new CreateFolderAction
				{
