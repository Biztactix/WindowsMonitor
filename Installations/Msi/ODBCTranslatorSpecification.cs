...
public String Version { get; private set; }
		...
		public static ODBCTranslatorSpecification[] Retrieve(string remote, string username, string password)
		{
		...
		public static ODBCTranslatorSpecification[] Retrieve()
		{
			var managementScope = new ManagementScope(new ManagementPath("root\\cimv2"));
			return Retrieve(managementScope);
		}

		public static ODBCTranslatorSpecification[] Retrieve(ManagementScope managementScope)
		{
		    var objectQuery = new ObjectQuery("SELECT * FROM Win32_ODBCTranslatorSpecification");
            var objectSearcher = new ManagementObjectSearcher(managementScope, objectQuery);
            var objectCollection = objectSearcher.Get();

			var list = new List<ODBCTranslatorSpecification>();

            foreach (ManagementObject managementObject in objectCollection)
            {
				list.Add(new ODBCTranslatorSpecification
				{
