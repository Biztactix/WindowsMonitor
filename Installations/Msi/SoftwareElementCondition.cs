...
public String Version { get; private set; }
		...
		public static SoftwareElementCondition[] Retrieve(string remote, string username, string password)
		{
		...
		public static SoftwareElementCondition[] Retrieve()
		{
			var managementScope = new ManagementScope(new ManagementPath("root\\cimv2"));
			return Retrieve(managementScope);
		}

		public static SoftwareElementCondition[] Retrieve(ManagementScope managementScope)
		{
		    var objectQuery = new ObjectQuery("SELECT * FROM Win32_SoftwareElementCondition");
            var objectSearcher = new ManagementObjectSearcher(managementScope, objectQuery);
            var objectCollection = objectSearcher.Get();

			var list = new List<SoftwareElementCondition>();

            foreach (ManagementObject managementObject in objectCollection)
            {
				list.Add(new SoftwareElementCondition
				{
