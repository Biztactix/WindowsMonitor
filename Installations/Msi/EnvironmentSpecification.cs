...
public String Version { get; private set; }
		...
		public static EnvironmentSpecification[] Retrieve(string remote, string username, string password)
		{
		...
		public static EnvironmentSpecification[] Retrieve()
		{
			var managementScope = new ManagementScope(new ManagementPath("root\\cimv2"));
			return Retrieve(managementScope);
		}

		public static EnvironmentSpecification[] Retrieve(ManagementScope managementScope)
		{
		    var objectQuery = new ObjectQuery("SELECT * FROM Win32_EnvironmentSpecification");
            var objectSearcher = new ManagementObjectSearcher(managementScope, objectQuery);
            var objectCollection = objectSearcher.Get();

			var list = new List<EnvironmentSpecification>();

            foreach (ManagementObject managementObject in objectCollection)
            {
				list.Add(new EnvironmentSpecification
				{
