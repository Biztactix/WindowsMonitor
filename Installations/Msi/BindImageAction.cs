...
public String Version { get; private set; }
		...
		public static BindImageAction[] Retrieve(string remote, string username, string password)
		{
		...
		public static BindImageAction[] Retrieve()
		{
			var managementScope = new ManagementScope(new ManagementPath("root\\cimv2"));
			return Retrieve(managementScope);
		}

		public static BindImageAction[] Retrieve(ManagementScope managementScope)
		{
		    var objectQuery = new ObjectQuery("SELECT * FROM Win32_BindImageAction");
            var objectSearcher = new ManagementObjectSearcher(managementScope, objectQuery);
            var objectCollection = objectSearcher.Get();

			var list = new List<BindImageAction>();

            foreach (ManagementObject managementObject in objectCollection)
            {
				list.Add(new BindImageAction
				{
