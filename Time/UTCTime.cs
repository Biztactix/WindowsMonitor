...
public UInt32 Year { get; private set; }
		...
		public static UTCTime[] Retrieve(string remote, string username, string password)
		{
		...
		public static UTCTime[] Retrieve()
		{
			var managementScope = new ManagementScope(new ManagementPath("root\\cimv2"));
			return Retrieve(managementScope);
		}

		public static UTCTime[] Retrieve(ManagementScope managementScope)
		{
		    var objectQuery = new ObjectQuery("SELECT * FROM Win32_UTCTime");
            var objectSearcher = new ManagementObjectSearcher(managementScope, objectQuery);
            var objectCollection = objectSearcher.Get();

			var list = new List<UTCTime>();

            foreach (ManagementObject managementObject in objectCollection)
            {
				list.Add(new UTCTime
				{
