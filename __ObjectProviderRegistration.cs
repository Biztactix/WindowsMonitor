...
namespace WindowsMonitor
{
    /// <summary>
    /// </summary>
    public sealed class __ObjectProviderRegistration
    {
        ...
        public static IEnumerable<__ObjectProviderRegistration> Retrieve(string remote, string username, string password)
        {
        ...
        public static IEnumerable<__ObjectProviderRegistration> Retrieve()
        {
            var managementScope = new ManagementScope(new ManagementPath("root\\cimv2"));
            return Retrieve(managementScope);
        }

        public static IEnumerable<__ObjectProviderRegistration> Retrieve(ManagementScope managementScope)
        {
            var objectQuery = new ObjectQuery("SELECT * FROM __ObjectProviderRegistration");
            var objectSearcher = new ManagementObjectSearcher(managementScope, objectQuery);
            var objectCollection = objectSearcher.Get();

            foreach (ManagementObject managementObject in objectCollection)
                yield return new __ObjectProviderRegistration
                {
